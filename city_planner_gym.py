...
class CityPlanningEnv(gym.Env):
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "budget": spaces.Box(
                    low=0, high=self.budget, shape=(), dtype=np.float32
                ),
            }
        )
    ...
    def _get_observation(self):
        ...
        return {"grid": self.grid.copy(), "budget": self.remaining_budget}
    ...
    def step(self, action):
        ...
        if (
            self.grid[row, col] == 0
            and self.remaining_budget >= self.building_costs[building_name]
        ):
            self.grid[row, col] = (
                building_type + 1
            )  # +1 to represent the building type in the grid
    ...
    def _calculate_reward(self, row, col, building_type):
        ...
        if building_type == 0:  # School
            ...
            reward += (
                self._check_nearby(row, col, 3, 0) * self.rewards["hospital_school"]
            )
        ...
        if building_type not in {3, 5}:
            reward += (
                self._check_nearby(row, col, 1, building_type)
                * self.penalties["same_building_adjacent"]
            )
